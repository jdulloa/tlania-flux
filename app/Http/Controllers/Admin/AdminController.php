...
class AdminController extends Controller
{
    ...
    public function ticketSoldReport(Request $request)
    {
        ...
        $userPicks = UserPick::paid()
            ->whereDate('created_at', '>=', $request->start_date)
            ->whereDate('created_at', '<=', $request->end_date)
            ->selectRaw("SUM(amount) as amount, DATE_FORMAT(created_at,'{$format}') as created_on")
            ->orderBy('created_on')
            ->groupBy('created_on')
            ->get();
        ...
        foreach ($dates as $date) {
            $data[] = [
                'created_on' => $date,
                'sold_amount' => getAmount($userPicks->where('created_on', $date)->first()?->amount ?? 0)
            ];
        }
