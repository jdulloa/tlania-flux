...
class GeneralSettingController extends Controller
{
    ...
    public function generalUpdate(Request $request)
    {
        $request->validate([
            ...
            'welcome_bonus'   => 'required|numeric|gte:0',
        ]);
    ...
    {
        $pageTitle = 'Coin Instruction';
        $timezones = json_decode(file_get_contents(resource_path('views/admin/partials/timezone.json')));
        return view('admin.setting.coin_instruction', compact('pageTitle', 'timezones'));
    }

    public function coinInstructionUpdate(Request $request)
    {
        $request->validate([
            'request_amount'      => 'required|numeric|gte:0',
            'request_instruction' => 'required',
        ]);

        $general                      = gs();
        $general->request_amount      = $request->request_amount;
        $general->request_instruction = $request->request_instruction;
        $general->save();
    ...
    public function systemConfigurationSubmit(Request $request)
    {
        ...
        $general->request_for_coin = $request->request_for_coin ? Status::ENABLE : Status::DISABLE;
    ...
    public function logoIconUpdate(Request $request)
    {
        $request->validate([
            ...
            'logo_alternative' => ['image', new FileTypeValidate(['jpg', 'jpeg', 'png'])],
            'logo_retina' => ['image', new FileTypeValidate(['jpg', 'jpeg', 'png'])],
        ...
        if ($request->hasFile('logo_alternative')) {
            try {
                fileUploader($request->logo_alternative, $path, filename: 'logo_alternative.png');
            } catch (\Exception $exp) {
                $notify[] = ['error', 'Couldn\'t upload the logo'];
                return back()->withNotify($notify);
            }
        ...
        if ($request->hasFile('logo_retina')) {
            try {
                fileUploader($request->logo_retina, $path, filename: 'logo_retina.png');
            } catch (\Exception $exp) {
                $notify[] = ['error', 'Couldn\'t upload the logo'];
                return back()->withNotify($notify);
            }
