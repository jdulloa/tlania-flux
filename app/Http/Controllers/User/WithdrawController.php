...
class WithdrawController extends Controller
{
    ...
    public function withdrawStore(Request $request)
    {
        ...
        if ($request->amount < $method->min_limit) {
            $notify[] = ['error', __('The requested amount is below the minimum allowed.')];
            return back()->withNotify($notify)->withInput($request->all());
        }
        if ($request->amount > $method->max_limit) {
            $notify[] = ['error', __('The requested amount exceeds the maximum allowed.')];
            return back()->withNotify($notify)->withInput($request->all());
        }
        ...
        if ($afterCharge <= 0) {
            $notify[] = ['error', __('Sufficient funds are required to cover all withdrawal charges.')];
            return back()->withNotify($notify)->withInput($request->all());
        }
    ...
    public function withdrawPreview()
    {
        ...
        $pageTitle = __('Withdrawal Preview');
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        $transaction->details = __('Withdrawal request through ') . $withdraw->method->name;
        ...
        $adminNotification->title = __('New withdrawal request from ') . $user->username;
        ...
        $notify[] = ['success', __('Withdrawal request was sent successfully.')];
