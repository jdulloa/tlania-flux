...
class LotteryController extends Controller
{
    ...
    public function pendingDraw()
    {
        ...
        return view('Template::user.lottery.pending_draw', compact('pageTitle', 'phases'));
    }

    public function winningHistory()
    {
        $pageTitle = __('Winning History');
    ...
    public function pick(Request $request, $id)
    {
        ...
        if ($request->phase_id != @$lottery->activePhase->id) {
            $notify[] = ['error', __('The draw ended, please try again with an active draw.')];
        ...
        if ($request->payment_via == 'balance') {
            ...
            $transaction->details      = __('Ticket purchase payment.');
        ...
        $notify[] = ['success', __('Ticket purchase successful. Cross those fingers!')];
        return back()->withNotify($notify);
    }
    ...
    private function pickingValidation($lottery, $request)
    {
        ...
        $message = [
            'phase_id.required'               => __('The draw field is required.'),
            'payment_via.in'                  => __('Payment should be made directly or via balance transfer.'),
            'ticket.min'                      => __('The ticket amount must be at least ') . $minimumTicket . '.',
            'ticket.*.normal_ball.required'   => __('Each ticket should have ') . $lottery->total_picking_ball . __(' normal balls selected.'),
            'ticket.*.normal_ball.size'       => __('The normal ball must be ') . $lottery->total_picking_ball . '.',
            'ticket.*.power_ball.required'    => __('Each ticket should have ') . $lottery->total_picking_power_ball . __(' power balls selected.'),
            'ticket.*.power_ball.size'        => __('The power ball must be ') . $lottery->total_picking_power_ball . '.',
            'ticket.*.normal_ball.*.between'  => __('The normal ball must be between ') . $lottery->ball_start_from . __(' and ') . $maximumNormalBallNumber . '.',
            'ticket.*.power_ball.*.between'   => __('The power ball must be between ') . $lottery->pw_ball_start_from . __(' and ') . $maximumPowerBallNumber . '.'
        ];
